...
async def llm_validate_payload(payload: dict, schema: dict):
    async with httpx.AsyncClient() as client:
        resp = await client.post(f"{settings.llm_endpoint}/validate", json={"payload": payload, "schema": schema})
...
async def llm_summarize_log(log: str) -> str:
    async with httpx.AsyncClient() as client:
        resp = await client.post(f"{settings.llm_endpoint}/summarize", json={"log": log})
        resp.raise_for_status()
